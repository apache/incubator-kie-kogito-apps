...
public class AggregatedLimeExplainer implements GlobalExplainer<CompletableFuture<Map<String, Saliency>>> {
    ...
    private static final int DEFAULT_NO_OF_SAMPLES = 10;
    ...
    public AggregatedLimeExplainer() {
        ...
        this.noOfSamples = DEFAULT_NO_OF_SAMPLES;
    }

    public AggregatedLimeExplainer(LimeExplainer limeExplainer, int noOfSamples) {
    ...
    public CompletableFuture<Map<String, Saliency>> explainFromMetadata(PredictionProvider model, PredictionProviderMetadata metadata) {
        List<PredictionInput> inputs = metadata.getDataDistribution().sample(noOfSamples); // sample inputs from the data distribution
