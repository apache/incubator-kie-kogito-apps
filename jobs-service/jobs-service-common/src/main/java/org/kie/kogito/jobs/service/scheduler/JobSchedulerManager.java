...
import static org.kie.kogito.jobs.service.repository.JobRepository.SortTerm.byCreated;
import static org.kie.kogito.jobs.service.repository.JobRepository.SortTerm.byFireTime;
import static org.kie.kogito.jobs.service.repository.JobRepository.SortTerm.byId;
...
public class JobSchedulerManager {
    ...
    public void doLoadJobDetails(ZonedDateTime fromFireTime, ZonedDateTime toFireTime, final int retries) {
        ...
        for (JobDetails jobDetails : jobsDetails.stream().filter(this::isNotScheduled).toList()) {
            try {
                scheduler.internalSchedule(jobDetails, initialLoading.get());
                LOGGER.debug("Loaded and scheduled job {}.", jobDetails);
            } catch (Throwable th) {
                throwable = th;
            }
        }
