...
jest.mock('../pmml/LinearRegressionViewer', () => ({
  LinearRegressionViewer: () => <></>
}));
...
describe('ModelDiagram', () => {
  test('renders a DMN model', () => {
    ...
    // investigating other ways to test for the emebedded dmn viewer
    expect(wrapper.find('iframe')).toHaveLength(1);
  });

  test('renders a PMML model', () => {
    const wrapper = mount(<ModelDiagram model={modelDataPMML} />);

    expect(wrapper.find('LinearRegressionViewer')).toHaveLength(1);
    expect(wrapper.find('LinearRegressionViewer').prop('xml')).toStrictEqual(
      modelDataPMML.model
    );
  });
