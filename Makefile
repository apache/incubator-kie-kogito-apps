...
build_chain_project='kiegroup/kogito-apps'
...
build-upstream: build-chain
	export BUILD_MVN_OPTS="${mvn_opts}" && build-chain build cross_pr -f ${build_chain_file} -o /tmp/bc -p ${build_chain_project} -b ${build_chain_branch} -g ${build_chain_group} --skipParallelCheckout --skipProjectExecution kiegroup/kogito-apps --skipProjectCheckout kiegroup/kogito-apps
