      ...
      for (const i in data) {
        ...
        if (typeof data[i] === 'object') {
          ...
          if (rest !== '__typename' && !rest.match('/ __typename')) {
            !tempKeys.includes(k + rest) && tempKeys.push(k + rest);
            if (rest.hasOwnProperty) {
              if (rest === list.processInstances[1]) {
                const tempObj = {
                  id: data.id,
                ...
                tempValue.push(ele);
              } else if (rest === list.processInstances[2]) {
                const ele = {
                  title: stateIcon(data[i].toString())
                };
                tempValue.push(ele);
              } else if (rest === list.processInstances[3]) {
                const ele = {
                  title: <Moment fromNow>{data[i].toString()}</Moment>
                };
                tempValue.push(ele);
              } else if (rest === list.processInstances[4]) {
                const ele = {
                  title: (
                    <>
  ...
  const initLoad = () => {
    if (columnFilters.length > 0) {
      columnFilters.map(item => {
        ...
        const finalMetaKeys = [];
        const tempList = list.processInstances.slice(1,5)
        tempList.map(ele => {
          ele = ele.replace(/([A-Z])/g, ' $1').trim().toLowerCase();
