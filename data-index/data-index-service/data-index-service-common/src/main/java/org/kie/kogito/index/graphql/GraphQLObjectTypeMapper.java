...
public class GraphQLObjectTypeMapper implements Function<DomainDescriptor, GraphQLObjectType> {
    ...
    public GraphQLObjectTypeMapper(GraphQLSchema schema, Map<String, GraphQLType> additionalTypes,
    ...
    private Consumer<GraphQLObjectType.Builder> build(DomainDescriptor domain) {
        return builder -> domain.getAttributes().forEach(field -> {
            ...
            if (KogitoMetadata.class.getName().equals(field.getTypeName())) {
                builder.field(newFieldDefinition().name(KOGITO_DOMAIN_ATTRIBUTE).type(schema.getObjectType("KogitoMetadata")))
                        .build();
            } else {
    ...
    private GraphQLOutputType getGraphQLType(AttributeDescriptor attribute, GraphQLSchema schema,
            Map<String, GraphQLType> additionalTypes, Map<String, DomainDescriptor> allTypes) {
        String typeName = getTypeName(attribute.getTypeName());
        GraphQLType type = schema.getType(typeName);
        if (type == null) {
            type = additionalTypes.computeIfAbsent(typeName,
                    k -> new GraphQLObjectTypeMapper(schema, additionalTypes, allTypes).apply(allTypes.get(typeName)));
        }
