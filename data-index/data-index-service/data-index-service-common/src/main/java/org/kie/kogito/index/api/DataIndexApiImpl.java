...
import org.kie.kogito.index.api.predicate.RetryProcessInstancePredicate;
...
public class DataIndexApiImpl implements DataIndexApi {
    ...
    public static final String GET_PROCESS_INSTANCE_NODES_URI = "/management/processes/%s/instances/%s/nodeInstances";
    ...
    public CompletableFuture<String> retryProcessInstance(String serviceURL, ProcessInstance processInstance) {
        ...
        if (new RetryProcessInstancePredicate().test(processInstance)) {
    ...
    @Override
    public CompletableFuture<List<NodeInstance>> getProcessInstanceNodes(String serviceURL, ProcessInstance processInstance) {
        String requestURI = format(GET_PROCESS_INSTANCE_NODES_URI, processInstance.getProcessId(), processInstance.getId());
        return sendGetClientRequest(getWebClient(serviceURL), requestURI, "Get Process Instance nodes with id: " + processInstance.getId(), List.class);
    }
