...
public class KogitoRuntimeClientImpl implements KogitoRuntimeClient {
    ...
    public static final String UPDATE_VARIABLES_PROCESS_INSTANCE_URI = "/%s/%s";
    public static final String TRIGGER_NODE_INSTANCE_URI = "/management/processes/%s/instances/%s/nodes/%s"; //node def
    public static final String RETRIGGER_NODE_INSTANCE_URI = "/management/processes/%s/instances/%s/nodeInstances/%s"; // nodeInstance Id
    public static final String CANCEL_NODE_INSTANCE_URI = "/management/processes/%s/instances/%s/nodeInstances/%s"; // nodeInstance Id
    ...
    public CompletableFuture<String> updateProcessInstanceVariables(String serviceURL, ProcessInstance processInstance, String variables) {
        String requestURI = format(UPDATE_VARIABLES_PROCESS_INSTANCE_URI, processInstance.getProcessId(), processInstance.getId());
    ...
    public CompletableFuture<String> triggerNodeInstance(String serviceURL, ProcessInstance processInstance, String nodeDefinitionId) {
        String requestURI = format(TRIGGER_NODE_INSTANCE_URI, processInstance.getProcessId(), processInstance.getId(), nodeDefinitionId);
    ...
    public CompletableFuture<String> retriggerNodeInstance(String serviceURL, ProcessInstance processInstance, String nodeInstanceId) {
        String requestURI = format(RETRIGGER_NODE_INSTANCE_URI, processInstance.getProcessId(), processInstance.getId(), nodeInstanceId);
        return sendPostClientRequest(getWebClient(serviceURL), requestURI,
    ...
    public CompletableFuture<String> cancelNodeInstance(String serviceURL, ProcessInstance processInstance, String nodeInstanceId) {
        String requestURI = format(CANCEL_NODE_INSTANCE_URI, processInstance.getProcessId(), processInstance.getId(), nodeInstanceId);
    ...
    protected CompletableFuture sendPutClientRequest(WebClient webClient, String requestURI, String logMessage, String jsonBody) {
        ...
        try {
            ...
            new DataIndexServiceException("FAILED: " + logMessage + " errorMEssage:" + e.getMessage());
        }
        return future;
    }
