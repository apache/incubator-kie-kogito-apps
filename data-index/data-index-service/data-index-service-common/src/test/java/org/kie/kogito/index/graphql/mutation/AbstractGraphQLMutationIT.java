 ...
 */
package org.kie.kogito.index.graphql.mutation;
...
public abstract class AbstractGraphQLMutationIT extends AbstractIndexingIT {
    ...
    private DataIndexApi dataIndexApiClient;

    @BeforeEach
    public void setup() throws Exception {
        protobufService.registerProtoBufferType(getTestProtobufFileContent());
        dataIndexApiClient = mock(DataIndexApi.class);
    ...
    void testProcessInstanceAbort() {
        ...
        checkOkResponse("{ \"query\" : \"mutation{ ProcessInstanceAbort ( processInstanceId: \\\"" + processInstanceId + "\\\")}\"}");
    ...
    void testProcessInstanceRetry() {
        ...
        checkOkResponse("{ \"query\" : \"mutation{ ProcessInstanceRetry ( processInstanceId: \\\"" + processInstanceId + "\\\")}\"}");
    ...
    @Test
    void testProcessInstanceNodes() {
        String processId = "travels";
        String processInstanceId = UUID.randomUUID().toString();
        KogitoProcessCloudEvent startEvent = getProcessCloudEvent(processId, processInstanceId, ACTIVE, null, null, null);
        indexProcessCloudEvent(startEvent);

        checkOkResponse("{ \"query\" : \"mutation{ GetProcessInstanceNodes ( processInstanceId: \\\"" + processInstanceId + "\\\") {id}}\"}");

        verify(dataIndexApiClient).getProcessInstanceNodes(eq("http://localhost:8080"),
                eq(getProcessInstance(processId, processInstanceId, 1, null, null)));
    }
    ...
    void testProcessInstanceDiagram() {
        ...
        checkOkResponse("{ \"query\" : \"mutation{ GetProcessInstanceDiagram ( processInstanceId: \\\"" + processInstanceId + "\\\")}\"}");
