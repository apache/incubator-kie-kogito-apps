...
class DomainQueryIT extends QueryTestBase<String, ObjectNode> {
    ...
    void test() {
        ...
        queryAndAssert(assertWithObjectNode(), storage, singletonList(in("traveller.firstName", asList("John", "Jane"))), null,
                null, null, processInstanceId1, processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(equalTo("traveller.firstName", "John")), null, null, null,
                processInstanceId1);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(greaterThan("traveller.age", 27)), null, null, null);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(greaterThanEqual("traveller.age", 27)), null, null, null,
                processInstanceId1, processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(lessThan("traveller.age", 27)), null, null, null);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(lessThanEqual("traveller.age", 27)), null, null, null,
                processInstanceId1, processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(between("traveller.age", 27, 28)), null, null, null,
                processInstanceId1, processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(isNull("traveller.age")), null, null, null);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(notNull("traveller.age")), null, null, null,
                processInstanceId1, processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(contains("_id", processInstanceId2)), null, null, null,
                processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage,
                singletonList(containsAny("_id", asList(processInstanceId1, processInstanceId2))), null, null, null,
                processInstanceId1, processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage,
                singletonList(containsAll("_id", asList(processInstanceId1, processInstanceId2))), null, null, null);
        queryAndAssert(assertWithObjectNode(), storage, singletonList(like("traveller.firstName", "*hn")), null, null, null,
                processInstanceId1);
        queryAndAssert(assertWithObjectNode(), storage,
                singletonList(and(asList(equalTo("traveller.firstName", "John"), equalTo("traveller.lastName", "Doe")))), null,
                null, null, processInstanceId1);
        queryAndAssert(assertWithObjectNode(), storage,
                singletonList(or(asList(equalTo("traveller.firstName", "John"), equalTo("traveller.firstName", "Jane")))), null,
                null, null, processInstanceId1, processInstanceId2);
        queryAndAssert(assertWithObjectNode(), storage,
                asList(equalTo("traveller.firstName", "John"), equalTo("traveller.lastName", "Toe")), null, null, null);

        queryAndAssert(assertWithObjectNodeInOrder(), storage,
                asList(in("traveller.firstName", asList("Jane", "John")), in("traveller.lastName", asList("Doe", "Toe"))),
                singletonList(orderBy("traveller.lastName", SortDirection.ASC)), 1, 1, processInstanceId2);
        queryAndAssert(assertWithObjectNodeInOrder(), storage, null,
                singletonList(orderBy("traveller.firstName", SortDirection.ASC)), null, null, processInstanceId2,
                processInstanceId1);
        queryAndAssert(assertWithObjectNodeInOrder(), storage, null, null, 1, 1, processInstanceId2);
        queryAndAssert(assertWithObjectNodeInOrder(), storage, null,
                asList(orderBy("traveller.firstName", SortDirection.DESC), orderBy("traveller.lastName", SortDirection.ASC)), 1,
                1, processInstanceId2);
    }
}
