...
public class ProcessInstanceQueryTest {
    ...
    void test() {
        ...
        queryAndAssert(assertWithId(), storage, singletonList(and(asList(lessThan("start", Instant.now().toEpochMilli()), lessThanEqual("start", Instant.now().toEpochMilli())))), null, null, null, processInstanceId, subProcessInstanceId);
        ...
        queryAndAssert(assertWithId(), storage, asList(isNull("roles"), isNull("end"), greaterThan("start", Instant.now().toEpochMilli()), greaterThanEqual("start", Instant.now().toEpochMilli())), null, null, null);
