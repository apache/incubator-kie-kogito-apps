...
class UserTaskInstanceQueryIT extends QueryTestBase<String, UserTaskInstance> {
    ...
    void setUp() {
        this.storage =
                new MongoStorage<>(mongoClientManager.getCollection(USER_TASK_INSTANCES_STORAGE, UserTaskInstanceEntity.class),
    ...
    void test() {
        ...
        UserTaskInstance userTaskInstance1 =
                TestUtils.createUserTaskInstance(taskId1, processInstanceId1, RandomStringUtils.randomAlphabetic(5),
                        UUID.randomUUID().toString(), RandomStringUtils.randomAlphabetic(10), "InProgress", 0L);
        UserTaskInstance userTaskInstance2 = TestUtils.createUserTaskInstance(taskId2, processInstanceId2,
                RandomStringUtils.randomAlphabetic(5), null, null, "Completed", 1000L);
        ...
        queryAndAssert(assertWithId(), storage,
                singletonList(greaterThan("started", Instant.now().plus(1, ChronoUnit.DAYS).toEpochMilli())), null, null, null);
        queryAndAssert(assertWithId(), storage,
                singletonList(greaterThanEqual("completed", Instant.now().minus(1, ChronoUnit.DAYS).toEpochMilli())), null,
                null, null, taskId1, taskId2);
        queryAndAssert(assertWithId(), storage,
                singletonList(lessThan("completed", Instant.now().minus(1, ChronoUnit.DAYS).toEpochMilli())), null, null, null);
        queryAndAssert(assertWithId(), storage,
                singletonList(lessThanEqual("started", Instant.now().plus(1, ChronoUnit.DAYS).toEpochMilli())), null, null,
                null, taskId1, taskId2);
        queryAndAssert(assertWithId(), storage, singletonList(between("completed",
                Instant.now().minus(1, ChronoUnit.DAYS).toEpochMilli(), Instant.now().plus(1, ChronoUnit.DAYS).toEpochMilli())),
        ...
        queryAndAssert(assertWithId(), storage,
                singletonList(containsAny("processInstanceId", asList(processInstanceId1, processInstanceId2))), null, null,
                null, taskId1, taskId2);
        queryAndAssert(assertWithId(), storage,
                singletonList(containsAll("processInstanceId", asList(processInstanceId1, processInstanceId2))), null, null,
                null);
        ...
        queryAndAssert(assertWithIdInOrder(), storage,
                asList(in("id", asList(taskId1, taskId2)),
