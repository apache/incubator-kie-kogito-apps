...
public class UserTaskInstanceQueryTest {
    ...
    @BeforeEach
    void setUp() {
        this.storage = (Storage<String, UserTaskInstance>) storageFactory.getStorage(USER_TASK_INSTANCES_STORAGE);
    }
    ...
    void test() {
        ...
        queryAndAssert(assertWithId(), storage, singletonList(greaterThan("started", Instant.now().toEpochMilli())), null, null, null);
        queryAndAssert(assertWithId(), storage, singletonList(greaterThanEqual("completed", Instant.now().toEpochMilli())), null, null, null, taskId1, taskId2);
        queryAndAssert(assertWithId(), storage, singletonList(lessThan("completed", Instant.now().toEpochMilli())), null, null, null);
        queryAndAssert(assertWithId(), storage, singletonList(lessThanEqual("started", Instant.now().toEpochMilli())), null, null, null, taskId1, taskId2);
        queryAndAssert(assertWithId(), storage, singletonList(between("completed", Instant.now().toEpochMilli(), Instant.now().plus(1, ChronoUnit.DAYS).toEpochMilli())), null, null, null, taskId1, taskId2);
