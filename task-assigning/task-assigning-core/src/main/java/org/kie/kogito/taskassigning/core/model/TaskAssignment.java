 ...
 *                                          <p>
 *                                          Here we basically declared that we want to build a CHAINED graph but also since the
 *                                          "userRange" is the first
 *                                          valueRangeProviderRef we are declaring that the User will be used as anchor.
 *                                          So the solver will always start by using an User as the head of the list and then
 *                                          it'll consider adding Tasks
 *                                          to the linked structure.
 *                                          <p>
 *                                          Additionally for the calculation of the scores, etc, given a TaskAssignment of a
 *                                          particular solution we'll want to know quickly
 ...
 *                                          <p>
 *                                          and we take e.g. D, then to get the assigned Employee1 the list must be iterated,
 *                                          but this probably not the best idea.
 *                                          <p>
 *                                          Solution, add a shadow variable and let the solver populate this variable when the
 *                                          solution is constructed.
 *                                          With the declaration below a shadow variable is defined for keeping a reference to
 *                                          the anchor. This shadow variable
 ...
 *                                          Convenient shadow variable is declared for having the startTimeInMinutes of a task
 *                                          already calculated.
 ...
 *                                             <p>
 *                                             So the variableListenerClass is invoked when the source variable is
 *                                             changed/assigned.
 */
@PlanningEntity
