...
2) A kafka broker and a persistence store (PostgreSQL or Redis).
...
### The kafka broker and the persistence store

You will have to provide a kafka instance as well as a persistence store (PostgreSQL or Redis). At the end of this readme, there is a link that contains a full end to end example. 
...
The trusty service depends on kafka and a persistence store (PostgreSQL or Redis) to persist the tracing information.
Enviroment variables can be used to set the information needed to connect to kafka and the persistence store, for example

```bash
export KAFKA_BOOTSTRAP_SERVERS=localhost:9092
# For PostgreSQL:
export QUARKUS_DATASOURCE_JDBC_URL=jdbc:postgresql://localhost:5432/trusty
