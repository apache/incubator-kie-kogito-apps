...
@ApplicationScoped
public class TriggerInsertSqlBuilderImpl implements TriggerInsertSqlBuilder<JsonType, PostgresField, PostgresPartitionField, PostgresMapping, PostgresContext> {

    private static final Logger LOGGER = LoggerFactory.getLogger(TriggerInsertSqlBuilderImpl.class);
    ...
    public String createInsertTriggerFunctionSql(final PostgresContext context) {
        ...
        sql.append(String.format(CREATE_INSERT_TRIGGER_FUNCTION_TEMPLATE,
                ...
                simpleMappings
                        .stream()
                        .map(TriggerInsertSqlBuilderImpl::buildTargetIdentityFieldSql)
                        .collect(Collectors.joining(", " + String.format("%n"))),
                fieldsSql));
    ...
    private static String buildTargetPartitionFieldsSql(final List<PostgresPartitionField> sourcePartitionFields) {
        ...
        return String.format(CREATE_INSERT_TRIGGER_WHEN_TEMPLATE,
                sourcePartitionFields.stream()
                        .map(TriggerInsertSqlBuilderImpl::buildTargetPartitionFieldSql)
